...
# Add parent directory to path to find analysis package
...
class PhysicsAnalyzer:
    ...
    def plot_all_results(self, save_dir="analysis/plots"):
        """Create comprehensive plots using centralized visualizer."""
        viz = ExperimentVisualizer(save_dir)
        
        # Create all plots using the visualizer
        viz.plot_cherenkov_linearity(self.beam, self.chere, self.results, show=False)
        viz.plot_scintillator_saturation(self.beam, self.scint, self.results, show=False)
        viz.plot_detector_comparison(self.beam, self.chere, self.scint, self.results, show=False)
        viz.plot_distributions(self.chere, self.scint, self.results, show=False)
        viz.plot_final_summary(self.beam, self.chere, self.scint, self.results, show=True)
        
        print(f"\nAll plots saved to: {viz.save_dir}")
